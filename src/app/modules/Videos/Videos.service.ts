...
const createIntoDb = async (
  ...
  if (!instructorId) throw new ApiError(httpStatus.UNAUTHORIZED, "Unauthorized access.");

  if (!thumbnailFile) throw new ApiError(httpStatus.BAD_REQUEST, "Thumbnail is required");

  // Upload thumbnail
  const uploadThumbnailResult = await fileUploader.uploadToCloudinary(thumbnailFile);
  ...
  if (videoFile) {
    ...
    videoDuration = await getVideoDurationFromBufferAlt(videoFile.buffer);
  }

  // Serial number based on existing videos
  const existingVideoCount = await prisma.videos.count({ where: { courseId } });
  ...
  // Update course videoCount and totalDuration
...
const deleteItemFromDb = async (id: string) => {
  ...
  await prisma.courses.update({
    ...
    data: { videoCount: videos.length, videoDuration: totalDuration },
  });
